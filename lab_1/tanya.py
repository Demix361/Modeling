...
def work():
    h = 0.001  # 10**-5 это хороший шаг для численных методов
    ...
    for i in range(0, n, 1):#output_step):
        print("|{:^9.5f}|{:^15.8f}|{:^15.8f}|{:^15s}|{:^15s}|".format(x_arr[i], y3[i][0], y3[i][1], output(y1[i]),
                                                                      output(y2[i])))
